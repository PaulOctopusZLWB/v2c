...
export const api = {
  ...
  // 记忆确认闭环:候选列表 + confirm(Ed25519 签名)/reject/defer/restore(z 撤销)。
  memoryCandidates: () => request<MemoryCandidates>("/api/memory/candidates"),
  confirmMemory: (id: string, edited_claim?: string) =>
    request<MemoryConfirmReceipt>(`/api/memory/${id}/confirm`, { method: "POST", body: JSON.stringify({ edited_claim: edited_claim ?? null }) }),
