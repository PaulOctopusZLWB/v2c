...
class PersistentCommandEmotionAdapter:
    """Keeps a --server emotion2vec wrapper resident: one audio path in, one emotion JSON line
    out, so the model loads once per extraction run instead of once per segment.

    Mirrors PersistentCommandEmbedAdapter's subprocess lifecycle: lazy-spawn on first classify, a
    bounded readline so a stalled server can't hang past timeout_seconds, and a prompt kill in
    close() (the server is stateless). The wire protocol is the emotion wrapper's: an input line
    ``{"segment_id", "audio_path"}`` produces an output line ``{"segment_id", "label", "scores"}``
    or ``{"segment_id", "error": ...}``.
    """

    def __init__(self, *, command: list[str], timeout_seconds: float = 3600.0) -> None:
        if not command:
            raise ValueError("emotion server command must not be empty")
