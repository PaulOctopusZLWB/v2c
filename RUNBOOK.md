...
4. Successfully synced checked candidates are rewritten as managed read-only receipt blocks.
5. `pcn memory verify` rechecks stored signed events, canonical signing body hashes, and owner hash-chain links.
6. `pcn memory verify` also rebuilds the trusted materialized memory card view and diffs it against `memory_cards`.
7. `pcn memory export --since ...` writes trusted `raw_event_json` rows as JSONL for exchange/backup.
8. `signed_events` stores `event_hash`, `owner_sequence`, `prev_event_hash`, `raw_event_json`, `signing_body_json`, and `trust_status`.
